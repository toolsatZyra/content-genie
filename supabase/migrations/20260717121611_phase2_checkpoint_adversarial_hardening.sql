-- Phase 2 checkpoint hardening: authoritative lifecycle guards and exact
-- Series creative inheritance discovered by the final cold review.

alter table public.series_releases
  add column narrator_gender public.narrator_gender not null default 'male',
  add column voice_version_id uuid not null
    default 'ec4e61a6-dc45-53d9-ba4b-fd5c7f267b2f';

alter table public.series_releases
  add constraint series_releases_voice_identity_fk
  foreign key (voice_version_id, narrator_gender)
  references public.voice_versions(id, gender) on delete restrict;

create index series_releases_voice_identity_idx
  on public.series_releases (voice_version_id, narrator_gender);
...
begin
  ...
  join public.voice_version_availability availability
    on availability.voice_version_id = release.voice_version_id
  ...
  if not found then
    raise exception 'active Series voice is unavailable' using errcode = '23503';
  end if;
  return new;
end;
$$;
...
create trigger episodes_pinned_voice_guard
before insert or update of pinned_series_release_id, workspace_id, series_id
on public.episodes
for each row execute function private.guard_episode_pinned_voice();
...
as $$
declare
  episode_state public.episode_workflow_state;
begin
  select e.workflow_state into episode_state
  from public.episodes e
  where e.workspace_id = new.workspace_id
    and e.id = new.episode_id
  for update;
  if not found or episode_state <> 'world_setup' then
    raise exception 'Episode creative configuration is read-only'
      using errcode = '55000';
  end if;
  return new;
end;
...
revoke all on function private.guard_configuration_creative_mutation()
from public, anon, authenticated;

create trigger episode_configuration_creative_mutation_guard
before update of narrator_gender, voice_version_id, voice_confirmed_by,
  voice_confirmed_at, look_version_id, look_confirmed_by, look_confirmed_at
on public.episode_configuration_candidates
for each row execute function private.guard_configuration_creative_mutation();
