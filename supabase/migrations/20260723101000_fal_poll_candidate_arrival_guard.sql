-- A polling worker claims by provider request and can race a signed webhook.
-- Recheck candidate arrival after taking the provider-request lock so a stale
-- poll result can never terminalize a World run that already has valid output.
...
begin
  ...
  -- Signed webhook recording locks this same provider request before inserting
  -- the candidate. Because this check runs only after that lock is acquired, it
  -- observes a candidate committed while the stale polling worker was waiting.
  if exists (
    select 1
