-- One security-invoker read model for the Living Cinema World, Preflight, and
-- Create chambers. Underlying forced-RLS policies remain authoritative.

create view public.creation_readiness_projections
...
select
  ...
  jsonb_build_object(
    ...
    'quote',(select jsonb_build_object('id',quote.id,'quoteHash',quote.quote_hash,
      ...
      'target40UsdBreached',quote.target_40usd_breached,'expiresAt',quote.expires_at,
