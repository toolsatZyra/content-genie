-- Retrying a recovered provider output can legitimately encounter the same
-- safe failure class and attempt number as an earlier, rolled-back recovery.
-- The ledger transition must remain authoritative even when its diagnostic
-- event is an exact replay.
