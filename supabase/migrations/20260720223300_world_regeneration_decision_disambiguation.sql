-- Qualify the decision column against the PL/pgSQL record variable used by
-- the exact regeneration-authority lookup.
...
begin
  ...
  select candidate.* into decision from private.world_asset_decisions candidate
  where candidate.command_id=request.command_id
    and candidate.decision='regenerate'
    and candidate.actor_user_id=request.requested_by
    and candidate.configuration_candidate_id=request.configuration_candidate_id;
