-- A fresh fenced World run can reproduce the same stable character, form, or
-- location identity with harmless punctuation differences in its display
-- label. Preserve the already-published Series label when the UUID and every
-- semantic identity field agree; genuine key, ownership, temple, or real-place
-- conflicts still fail closed in the existing candidate commands.
