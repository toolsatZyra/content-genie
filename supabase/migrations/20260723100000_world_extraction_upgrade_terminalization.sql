-- An in-flight run that already recorded immutable v1/v2 extraction evidence
-- cannot overwrite it with v3. Return an explicit terminal upgrade signal so
-- the application opens a fresh World run instead of exhausting same-run
-- retries.
