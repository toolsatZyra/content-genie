-- Reconstruct provider slots in the same semantic order used by the immutable
-- plan materializer. Alphabetical slot-key order changed the JSON array order
-- and therefore produced a false plan-hash mismatch on every retry.
...
begin
  ...
  select jsonb_build_object(
    ...
    ) order by slot.shot_number,case slot.slot_kind
      when 'primary' then 1 when 'candidate' then 2 when 'retry' then 3 else 4 end)
