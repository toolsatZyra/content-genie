...
| TM-12 | Malicious/polyglot/oversized media exploits parser or worker, or a stolen task/broker identity crosses scope | I/A/C/$ | Critical | Quarantine, magic sniff, sandbox, limits, scan, re-encode; separate project service identity plus single-attempt capability grant, micro/production authority and fencing | Corpus of bombs/malformed media plus wrong client/project/environment/key/workspace/run/stage/fence/object/RPC, replayed assertion/grant `jti`, expiry, revocation, and cross-link tests |
...
| TM-22 | User/agent creates unbounded generations or retry loop, or a micro envelope claims production work | $/A | Critical | Separate micro/production authorities, exact slot types, sole reservations, hard ceilings, candidate/retry/time limits, circuit breaker | Concurrent duplicate commands, micro-to-video/render scope attempts, cross-envelope slot claims, and persistent retryable failures |
...
| TM-35 | Preview/dev environment or one Trigger project accesses another environment/project's data, broker identity, or providers | C/I/$ | Critical | Separate projects/keys, environment and broker-client assertions, project-bound public-key registry, no prod secrets | Run preview with production-shaped IDs; swap Trigger client/key/assertion/grant across project/environment; verify denial before provider key use |
